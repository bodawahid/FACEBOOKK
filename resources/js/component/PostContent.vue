    ...
    <transition-group name="fade" tag="ul">
     ...
     in $root.current_posts" class="card w-100 shadow-xss rounded-xxl border-0 p-4 mb-3">
            <span v-if="post.post.SharedPostId == null">
                ...
                <p class="fw-800 text-dark-500 lh-26 mt-2 font-xssss w-100"
            ...
            <span v-else>
                ...
                <span v-if="!post.SharedPost.deleted_at">
                    ...
                    <p class="fw-800 text-dark-500 mt-2 lh-26 font-xssss w-100"
                        :style="'text-align:' + (detectLanguage(post.post.content) ? 'right' : 'left')">
                        {{ post.post.content ?? '' }}
                        <!-- <a href="#" class="fw-600 text-primary ms-2">See more</a> -->
                    </p>
                    <div class="shared-post-container mb-3 p-3 bg-light rounded-3">
                        ...
                        <div class="d-flex align-items-center mb-2">
                            <figure class="avatar me-3">
                                <img :src="'storage/users/image/' + (post.SharedPost.profile_picture ?? 'default121000000.jpg')"
                                    alt="image" class="shadow-sm rounded-circle w45">
                            </figure>
                            <h5 class="fw-700 text-grey-900 font-xssss">
                                <a href="#" @click.prevent="goToProfile(post.SharedPost.user_id)" class="text-primary">
                        ...
                        <!-- <div class="card-body p-0 me-lg-5"> -->
                        <p class="fw-800 text-dark-500 lh-26 font-xssss w-100"
                            :style="'text-align:' + (detectLanguage(post.SharedPost.content) ? 'right' : 'left')">
                            {{ post.SharedPost.content ?? '' }}
                            <!-- <a href="#" class="fw-600 text-primary ms-2">See more</a> -->
                        </p>
                        <!-- </div> -->
                        ...
                        <!-- Shared Post Media -->
                        <div class="card-body d-block p-0 mb-3"
                            v-if="post.SharedPost.SharedPostMedia.length == 1 && post.SharedPost.SharedPostMedia[0].SharedPostMediaType == 'image'">
                            <div class="row ps-2 pe-2">
                                <div class="col-sm-12 p-1"><a href="images/t-30.jpg" data-lightbox="roadtr"><img
                                            :src="'storage/posts/media/' + post.SharedPost.SharedPostMedia[0].SharedPostMediaPath"
                                            class="rounded-3 w-100" alt="image"></a></div>
                            </div>
                        </div>
                        <!-- if videos are required here-->
                        <div v-else-if="post.SharedPost.SharedPostMedia.length == 1 && post.SharedPost.SharedPostMedia[0].SharedPostMediaType == 'video'"
                            class="card-body p-0 mb-3 rounded-3 overflow-hidden">
                            <a href="default-video.html" class="video-btn">
                                <video autoplay loop class="float-right w-100">
                                    <source
                        ...
                        <!-- {{ --post images more than one image here ... --}} -->
                        <div class="card-body d-block p-0" v-else-if="post.SharedPost.SharedPostMedia.length > 1">
                            {{ console.log('hello from here') }}
                            <div class="row">
                                <div v-for="(image, index) in post.SharedPost.SharedPostMedia.slice(0, 3)" :key="index"
                                    class="col-xs-4 col-sm-4 p-1 position-relative">
                                    <span v-if="image.SharedPostMediaType === 'image'" style="position: relative;">
                                        <!-- Link to view the image in lightbox -->
                                        <a :href="'images/t-12.jpg'" data-lightbox="roadtrip"
                                            class="position-relative d-block">
                                            <img :src="'storage/posts/media/' + image.SharedPostMediaPath"
                                                class="rounded-3 w-100" alt="image">

                                            <!-- Show the number on the 3rd image (or any condition) -->
                                            <div v-if="index === 2 && post.SharedPost.SharedPostMedia.length - 3 > 0"
                                                class="position-absolute top-50 left-50 start-0  bg-dark text-white p-2 rounded-circle">
                                                + {{ post.SharedPost.SharedPostMedia.length - 3 }}
                                            </div>
                                        </a>
                                    </span>
                                </div>
                ...
                <div v-else class="card-body p-0 d-flex">
                    ...
                    <p>
                        عندما يحدث هذا، عادةً ما يكون بسبب قيام منشئ المنشور بمشاركته مع مجموعة صغيرة من الأشخاص فقط، أو
                        تغيير الأشخاص الذين يمكنهم رؤيته، أو أنه قد تم حذفه.
                    </p>
                </div>
            </span>
            ...
            <div class="card-body d-flex p-0 mt-3">
                ...
                <div class="dropdown-menu dropdown-menu-end p-4 rounded-xxl border-0 shadow-lg"
                    ...
                    <a class="btn fw-700 font-xssss mt-4 text-grey-500 d-flex align-items-center mb-3" type="button"
                        data-bs-toggle="modal" data-bs-target="#sharePostModal" @click.prevent="setSharedId(post.post.id)">Share
                        On Your
                        Profile</a>
